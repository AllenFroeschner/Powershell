...
FUNCTION findFile{
    ...
    else
    {
        ...
        Write-Host "File already exists: ",$file[$i] -ForegroundColor "Green" 
...
Write-Host $text[$indx] -ForegroundColor "Red"
[string[]]$array = Get-Content -Path "$($curDir)\files\List\names1.txt"; Write-Host "Computers in List: ", $array.length -BackgroundColor DarkCyan
[string[]]$arrayOffline = Get-Content -Path "$($curDir)\files\Log\OFFLINE.txt"; Write-Host "Number of Logged offline: ", $arrayOffline.length -BackgroundColor DarkCyan
[string[]]$arrayDev = Get-Content -Path "$($curDir)\files\List\names1.txt"; Write-Host "Number of company devices TD: ", $arrayDev.length -BackgroundColor DarkCyan
...
for($i=$start; $i -lt $array.length; $i++){
        ...
        if (Test-Connection -BufferSize 32 -Count 1 -ComputerName $CompName -Quiet) {
        	...
        	"CPU: " + $computerCPU.Name
                ...
                Catch {
                    Write-Output "Drive not found trying legacy method"
                    NotFound($CompName)
                }
        	...
        	"Last Reboot: " + $computerSystem.LastBootUpTime
        	""
        	"-------------------------------------------------------"
        } 
        else {
    ...
    switch($enter1){
		...
		$computerSystem = get-wmiobject Win32_ComputerSystem -Computer $CompName
    		...
    		$computerHDD = Get-WmiObject Win32_LogicalDisk -ComputerName $CompName -Filter drivetype=3 -ErrorVariable NotFound
                ...
                "zip bomb" {
                       for($i=0; $i -lt 5;$i++){
                       Restart-Computer -ComputerName $compName -Credential $Creds -Wait -For PowerShell -Timeout 60 -Delay 2 -Force
                       }                    
                ...
                "h" { "","PSSession= remote powershell session;","free space= check free space in C drive;","remote session= not working just use PSSession","Disconnect= disconnects from remote pc","zip bomb: dont do that","compinfo: gets info about computer","";
                }
        ...
        "h" { "","1= check office install version of full list; ","2= check office install version single computer; ","clearLog= clear log of offline computers;","select computer= connect to computer and perform commands;","reset password= reset AD user password","compinfoall: gets info about all computers","CompInfoAllToFile: sends data to log","";
                }
    }
}
}
