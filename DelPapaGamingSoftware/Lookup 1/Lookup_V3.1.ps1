...
FUNCTION findFile{
    ...
    $file = 'C:\DelPapa Gaming Software\files\Log\OFFLINE.txt','C:\DelPapa Gaming Software\files\List\computers.txt','C:\DelPapa Gaming Software\files\List\names.txt','C:\DelPapa Gaming Software\files\List\names1.txt'
...
FUNCTION createLogFile{
    param($CompName,$incr)
    $file = 'C:\DelPapa Gaming Software\files\Log\computerInfo'+[string]$CompName
    $temp=$file+$incr+'.txt'
    "tempfile:", $temp
...
[string[]]$array = Get-Content -Path 'C:\DelPapa Gaming Software\files\List\names1.txt'; Write-Host "Computers in List: ", $array.length
[string[]]$arrayOffline = Get-Content -Path 'C:\DelPapa Gaming Software\files\Log\OFFLINE.txt'; Write-Host "Number of Logged offline: ", $arrayOffline.length
[string[]]$arrayDev = Get-Content -Path 'C:\DelPapa Gaming Software\files\List\computers.txt'; Write-Host "Number of company devices TD: ", $arrayDev.length
...
FUNCTION Full_List {
param($array)
"EMPTY `rEMPTY" | Out-File -FilePath C:\DelPapa Gaming Software\Log\OFFLINE.txt
...
for($i=$start; $i -lt $array.length; $i++){
        ...
        if (Test-Connection -BufferSize 32 -Count 1 -ComputerName $RUser -Quiet) {
            ...
            "The remote machine is Down"
             $RUser | Out-File -FilePath C:\DelPapa Gaming Software\Log\OFFLINE.txt -Append
        }
...
while($App=$true){
    ...
    switch($enter1){
        1 {
            ...
            $OfflineOrOnline = Read-Host -Prompt 'Do you want to only search offline computers? (Y/N)'
                ...
                2 { 
                    ...
                    "The remote machine is Down"
                    $enter | Out-File -FilePath C:\DelPapa Gaming Software\files\Log\OFFLINE.txt -Append
                    }
                }
        ...
        "clearLog"{"EMPTY `rEMPTY" | Out-File -FilePath C:\DelPapa Gaming Software\files\Log\OFFLINE.txt}
